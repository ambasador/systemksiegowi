...
@section("htmlheader_title", "Edycja roli : ".$role->$view_col)
			...
			<div class="col-md-8 col-md-offset-2">
				{!! Form::model($role, ['route' => [config('laraadmin.adminRoute') . '.roles.update', $role->id ], 'method'=>'PUT', 'id' => 'role-edit-form']) !!}
					...
					<div class="form-group">
						{!! Form::submit( 'Aktualizuj', ['class'=>'btn btn-success']) !!} <button class="btn
						btn-default
						pull-right"><a href="{{ url(config('laraadmin.adminRoute') . '/roles') }}">Anuluj</a></button>
					</div>
				{!! Form::close() !!}
			</div>
		</div>
