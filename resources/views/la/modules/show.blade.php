...
@section('htmlheader_title', 'Widok modułu')
...
<div id="page-content" class="profile2">
	...
	@endif
		<div class="col-md-4">
			<div class="row">
				...
				<div class="col-md-9">
					<a class="text-white" href="{{ url(config('laraadmin.adminRoute') . '/'.$module->name_db) }}"><h4 data-toggle="tooltip" data-placement="left" title="Otwórz {{ $module->model }} Moduł" class="name">{{ $module->label }}</h4></a>
					<div class="row stats">
						<div class="col-md-12">{{ Module::itemCount($module->name) }} rekordy</div>
					</div>
		...
		<div class="col-md-4">
			@if($module->view_col != "")
				@if(isset($module->is_gen) && $module->is_gen)
					<div class="dats1 text-center"><a data-toggle="tooltip" data-placement="left" title="Aktualizuj moduł" class="btn btn-sm btn-success" style="border-color:#FFF;" id="generate_update" href="#"><i class="fa fa-refresh"></i> Aktualizuj moduł</a></div>
					<div class="dats1 text-center"><a data-toggle="tooltip" data-placement="left" title="Zaktualizuj plik migracji" class="btn btn-sm btn-success" style="border-color:#FFF;" id="update_migr" href="#"><i class="fa fa-database"></i> Aktualizuj migracje</a></div>
				@else
					<div class="dats1 text-center"><a data-toggle="tooltip" data-placement="left" title="Wygeneruj migrację + CRUD" class="btn btn-sm btn-success" style="border-color:#FFF;" id="generate_migr_crud" href="#"><i class="fa fa-cube"></i> Wygeneruj migrację + CRUD</a></div>
					
 					<div class="dats1 text-center"><a data-toggle="tooltip" data-placement="left" title="Generate Migration File" class="btn btn-sm btn-success" style="border-color:#FFF;" id="generate_migr" href="#"><i class="fa fa-database"></i> Wygeneruj migrację</a></div>
				@endif
			@else
				<div class="dats1 text-center">Aby wygenerować migrację lub CRUD, ustaw kolumnę widoku za pomocą ikony <i class = 'fa fa - eye'> </i> obok kolumny</div>
			@endif
		</div>
	...
	<ul id="module-tabs" data-toggle="ajax-tab" class="nav nav-tabs profile" role="tablist">
		...
		<li class="tab-pane" id="fields">
			<a id="tab_fields" role="tab" data-toggle="tab" class="tab_info active" href="#fields"
			   data-target="#tab-info"><i class="fa fa-bars"></i> Pola modułu</a>
		</li>
		
		<li class="tab-pane" id="access">
			<a id="tab_access" role="tab" data-toggle="tab"  class="tab_info " href="#access"
			   data-target="#tab-access"><i class="fa fa-key"></i> Dostęp</a>
		</li>
		
		<li class="tab-pane" id="sort">
			<a id="tab_sort" role="tab" data-toggle="tab"  class="tab_info " href="#sort" data-target="#tab-sort"><i
						class="fa fa-sort"></i> Sortuj</a>
		</li>
		
		<a data-toggle="modal" data-target="#AddFieldModal" class="btn btn-success btn-sm pull-right btn-add-field"
		   style="margin-top:10px;margin-right:10px;">Dodaj pole</a>
	</ul>
	...
	<div class="tab-content">
						...
						<tbody>														
							@foreach ($module->fields as $field)
								<tr>
									...
									<td>@if($field['unique']) <span class="text-danger">Prawda</span>@endif </td>
									<td>{{ $field['defaultvalue'] }}</td>
									<td>{{ $field['minlength'] }}</td>
									<td>{{ $field['maxlength'] }}</td>
									<td>@if($field['required']) <span class="text-danger">Prawda</span>@endif </td>
		...
		<div role="tabpanel" class="tab-pane fade in p20 bg-white" id="tab-access">
			<div class="guide1">
				<span class="pull-left">Dostęp do modułu dla ról</span>
				<i class="fa fa-circle gray"></i> Niewidzialny <i class="fa fa-circle orange"></i> Read-Only <i
						class="fa fa-circle green"></i> Zapisz
			</div>
				...
				<table class="table table-bordered dataTable no-footer table-access">
					<thead>
						<tr class="blockHeader">
							<th width="14%">
								<input class="alignTop" type="checkbox" id="role_select_all" >&nbsp; Role
							</th>
							<th width="14%">
								<input type="checkbox" id="view_all" >&nbsp; Widok
							</th>
							<th width="14%">
								<input type="checkbox" id="create_all" >&nbsp; Utwórz nowy
							</th>
							<th width="14%">
								<input type="checkbox" id="edit_all" >&nbsp; Edytuj
							</th>
							<th width="14%">
								<input class="alignTop" type="checkbox" id="delete_all" >&nbsp; Usuń
							</th>
							<th width="14%">Uprawnienia</th>
						</tr>
					</thead>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">Usunięcie modułu</h4>
			</div>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel">Dodaj {{ $module->model }} pola</h4>
			</div>
			...
			<div class="modal-body">
				<div class="box-body">
					<div class="form-group">
						...
						{{ Form::text("label", null, ['class'=>'form-control', 'placeholder'=>'Etykieta pola', 'data-rule-minlength' => 2, 'data-rule-maxlength'=>20, 'required' => 'required']) }}
					</div>
					
					<div class="form-group">
						<label for="colname">Nazwa kloumny :</label>
						{{ Form::text("colname", null, ['class'=>'form-control', 'placeholder'=>'Nazwa kolumn (małe litery)', 'data-rule-minlength' => 2, 'data-rule-maxlength'=>20, 'data-rule-banned-words' => 'true', 'required' => 'required']) }}
					</div>
					...
					<div id="length_div">
						<div class="form-group">
							<label for="minlength">Minimalna wartość :</label>
							{{ Form::number("minlength", null, ['class'=>'form-control', 'placeholder'=>'Minimalna wartość']) }}
						</div>
						
						<div class="form-group">
							<label for="maxlength">Maksymalna worstość :</label>
							{{ Form::number("maxlength", null, ['class'=>'form-control', 'placeholder'=>'Maksymalna worstość']) }}
						</div>
					</div>
					...
					<div class="form-group values">
						...
						<div class="radio" style="margin-bottom:20px;">
							<label>{{ Form::radio("popup_value_type", "table", true) }} z tabeli sql</label>
							<label>{{ Form::radio("popup_value_type", "list", false) }} z listy</label>
						</div>
						{{ Form::select("popup_vals_table", $tables, "", ['id'=>'popup_vals_table', 'class'=>'form-control', 'rel' => '']) }}
						
						<select id="popup_vals_list" class="form-control popup_vals_list" rel="taginput" multiple="1" data-placeholder="Dodaj wiele wartości (naciśnij klawisz Enter, aby dodać)" name="popup_vals_list[]">
