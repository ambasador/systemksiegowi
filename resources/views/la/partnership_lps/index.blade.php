...
@section("contentheader_title", "Społki Partnership LLP")
@section("contentheader_description", "Lista")
@section("section", "Partnership LPs")
@section("sub_section", "Lista")
@section("htmlheader_title", "Lista")

@section("headerElems")
	@la_access("Partnership_LPs", "create")
	<button class="btn btn-success btn-sm pull-right" data-toggle="modal" data-target="#AddModal">Dodaj społkę Partnership LLP</button>
	@endla_access
@endsection
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">Dodaj społkę Partnership LLP</h4>
				</div>
				{!! Form::open(['action' => 'LA\Partnership_LPsController@store', 'id' => 'partnership_lp-add-form']) !!}
...
<script>
    $(function () {
        ...
        $registration_date.on('keyup change blur', function(){
            var date = $(this).val();
            $first_set_period.date;
            var set21Months = moment(date, "DD-MM-YYYY").add('months', 21).endOf('month');
            var set1Month = moment(date, "DD-MM-YYYY").add('months', 1).startOf('month');
            var set12Months = moment(date, "DD-MM").add('months', 12).subtract('days', 1).endOf('month');
            var set9Months = moment($ends_hidden.val(), "DD-MM").add('months', 9);
            ...
            $Eof_first_settlement.val(day + '-' + month + '-' + year);
            $b_c_billing_periods.val((day1 + '/' + month1));
            $ends_hidden.val(day12 + '-' + month12 + '-' + year12);
        ...
        $("#example1").DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url(config('laraadmin.adminRoute') . '/partnership_lp_dt_ajax') }}",
            language: {
        ...
        $("#partnership_lp-add-form").validate({
            rules: {
                registration_date:{
                    validDate: true
                },
