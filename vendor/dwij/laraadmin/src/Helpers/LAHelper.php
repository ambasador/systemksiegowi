...
class LAHelper
{
	...
	public static function print_menu($menu, $active = false) {
		...
		$str = '<li'.$treeview.' '.$active_str.'><a href="'.url(config("laraadmin.adminRoute") . '/' . $menu->url ) .'"><i class="fa '.$menu->icon.'"></i> <span>'.LAHelper::real_module_name($menu->real_name).'</span> '.$subviewSign.'</a>';
    ...
    // LAHelper::print_menu_dashboard($menu)
    public static function print_menu_dashboard($menu, $active = false) {
        $childrens = \Dwij\Laraadmin\Models\Menu::where("parent", $menu->id)->orderBy('hierarchy', 'asc')->get();

        $treeview = "";
        $subviewSign = "";
        if(count($childrens)) {
            $treeview = " class=\"col-md-6\"";
            $subviewSign = '<i class="fa fa-angle-left pull-right"></i>';
        }
        $active_str = '';
        if($active) {
        ...
        $str = '<div class="col-md-4 '.LAHelper::real_module_name($menu->name).'"><div class="box"><a href="'.url(config("laraadmin.adminRoute") . '/' . $menu->url ) .'"><h3 class="box-title"><i class="fa '.$menu->icon.'"></i> <span
>'.LAHelper::real_module_name($menu->real_name).'</span> 
'.$subviewSign.'</h3></a>';
...
//        }
	...
	public static function print_menu_topnav($menu, $active = false) {
		...
		$str = '<li '.$treeview.''.$active_str.'><a '.$treeview2.' href="'.url(config("laraadmin.adminRoute") . '/' . $menu->url ) .'">'.LAHelper::real_module_name($menu->real_name).$subviewSign.'</a>';
